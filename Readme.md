install.packages(c(
  "ape", "TreeSim", "mvMORPH", "phytools", "phangorn",
  "adephylo", "vegan", "labdsv", "compositions",
  "Matrix", "MASS", "cluster", "dplyr", "tidyr", "getopt"
))
🚀 Usage
...
🔬 Workflow Summary
Simulate host phylogeny using the BD model.
Generate microbial abundance matrix with covariance structure defined by exponent.
Transform abundance data via CLR and phylogenetic correction.
Perform PCoA for multiple distance metrics.
Evaluate clustering and correspondence with the true covariance (Rho).
Save all outputs to the chosen directory.
🧠 Example Summary
Example command:
...
Sishuo Wang — Code development, simulation implementation, conceptualization, and major updates
(2024–2025)
