...
plot_graphs <- function(m1, m2, m3) {
    ...
    create_single_plot <- function(matrix, method, color, title) {
        ...
        if (method == "euclidean") {
            ...
            #diss <- dsvdis(matrix, index = method)
            diss <- vegdist(matrix, method = method)
        }
...
round(Q[, column_sorted_ind_2], rounded_to)
...
cat("\n\n\n")
pcoa_1 <- calculate_pcoa(YY, dist_method, is_standardize)

if(is_inter){
    pcoa_2 <- calculate_pcoa(Y, dist_method)
    plot_graphs(Q, Y, dist_method)
} else{
