...
# XQ
# for empirical data, abun == prop since only prop is provided
...
# not useful any more, as we use CLR, which uses all bacterial taxa
ref_index <- dim(prop)[2]
log_prop <- log(prop+1e-6)
row_geomean_log <- rowMeans(log_prop)
log_prop_geomean <- matrix(nrow=tnum, ncol=bnum)

for(i in 1:dim(prop)[1]) # iterate host
{
    log_prop_geomean[i,] <- log(prop[i,]) - row_geomean_log[i]
    prop[i,] <- prop[i,]/sum(prop[i,]) # make Q rela abundance
}
...
if(! is_inter){
    ...
    plot_graphs(prop, log_prop_geomean, P)
}
