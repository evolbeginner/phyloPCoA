...
generate_metadata2 <- function(metadata_file, abundance){
    ...
    merged_data <- merged_data %>%
        ...
        ungroup()
    #write.table(merged_data, "prop_abundance.txt"); q()
...
do_sim <- function(tnum=30, bnum=8){
    # Do simulation
    # BD model XQ
    #trees <- sim.bd.taxa.age(tnum, 1, lambda=1, mu=0, rho=1, age=1); tree <- trees[[1]]
...
#print(abundance)
#phylo_sig <- filter_brown(abundance, tree); q()
#print(phylo_sig$P)
