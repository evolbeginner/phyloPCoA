...
simulate_covariance <- function(n, rate = 1) {
	# Draw standard deviations (σ_i ~ Exp(rate))
	sigma <- rexp(n, rate = rate)
	# Draw random correlations uniformly in [-1, 1]
	Rho <- matrix(runif(n * n, -1, 1), n, n)
	Rho <- (Rho + t(Rho)) / 2  # make symmetric
	diag(Rho) <- 1             # set diagonal to 1
	# Construct covariance matrix: Σ = D * Rho * D
	D <- diag(sigma)
	Sigma <- D %*% Rho %*% D
...
do_sim <- function(tnum, bnum, lambda, mu, rho, age){
    ...
    R <- simulate_covariance(bnum, 1/scale_R)
...
create_single_plot <- function(matrix, method = "bray", color = "black", title = "PCoA Plot", group_samples = NULL){
  # Check input type
	f (!is.matrix(matrix) && !is.data.frame(matrix)) {
    stop("Input must be a matrix or data.frame.")
  }
  
  # Compute distance
	iss <- vegan::vegdist(matrix, method = method)
  
  # Perform PCoA (cmdscale)
	ts <- cmdscale(diss, k = 2, eig = TRUE)
  
  # Extract coordinates correctly
	oords <- as.data.frame(pts$points)
	f (ncol(coords) < 2) {
    stop("PCoA returned less than two dimensions — check your input data.")
  }
	olnames(coords) <- c("PC1", "PC2")
	oords$Sample <- rownames(matrix)
	oords$Color <- color  # default color
  
  # Apply group colors if provided
	f (!is.null(group_samples) && is.list(group_samples)) {
  ...
  # Add sample labels
	ext(coords$PC1, coords$PC2,
       labels = coords$Sample,
       pos = 4,
       cex = 0.8)
  
    f (!is.null(group_samples)) {
    for (grp in group_samples) {
